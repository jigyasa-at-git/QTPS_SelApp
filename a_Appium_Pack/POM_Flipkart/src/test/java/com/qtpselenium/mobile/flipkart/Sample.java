...
import com.qtpselenium.mobile.flipkart.pages.FilterPage;
...
public class Sample {
			   ...
			   if(page instanceof HomePage){
				   ...
				   for(int l3=0;l3<level3Menu.length;l3++){	
					   ...
					   if(page instanceof ProductSelectionPage){
						   handleProductSelectionPage((ProductSelectionPage)page);
					   }else if(page instanceof ProductDisplayPage){
						   handleProductDisplayPage((ProductDisplayPage)page);
					   }
				   }
	...
	public static void handleProductSelectionPage(ProductSelectionPage page){
		System.out.println("in productselection page");
		//page.menu.swipeAndOpenMenu();
		ProductDisplayPage pdPage = page.randomSelectItem();
		
		handleProductDisplayPage(pdPage);
	}
	
	public static void handleProductDisplayPage(ProductDisplayPage page){
		System.out.println("in productdisplay page");
		
		// scripts
		FilterPage filterPage = page.gotoFilters();
		ProductDisplayPage pdPage=filterPage.filterOnBrand();
		pdPage.verifyProductNames();
		filterPage = page.gotoFilters();
